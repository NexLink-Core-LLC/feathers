...
// DefaultLocation is the default path to the configuration file. Its value is
// platform-specific and defined in paths_unix.go / paths_windows.go.
...
func NewAtPath(path string) (*Configuration, error) {
	...
	// Apply any platform-specific default overrides (e.g. Windows uses
	// C:\Pterodactyl\... paths instead of the Linux /var/lib/pterodactyl
	// defaults). Values from the config file or Panel still take priority since
	// they are unmarshalled over these defaults.
