...
// SetImpersonator is a no-op on Unix: process/file isolation there is handled by
// Docker (containers + namespaces), not by impersonating a Windows account. It
// exists so the OS-neutral server/filesystem layer can call it unconditionally.
