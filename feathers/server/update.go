...
func (s *Server) SyncWithEnvironment() {
	...
	// Environments that carry an image label and stop configuration (Docker,
	// winproc) get those updated here. The capability is expressed as an
	// interface so this code does not depend on a specific backend.
	if e, ok := s.Environment.(interface {
		SetImage(string)
		SetStopConfiguration(remote.ProcessStopConfiguration)
	}); ok {
		s.Log().Debug("syncing image and stop configuration with environment")
