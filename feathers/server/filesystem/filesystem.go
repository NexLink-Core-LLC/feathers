...
// SetImpersonator wires an impersonation wrapper into the underlying filesystem
// so that on Windows each disk syscall runs as the server's restricted account.
// It is a no-op on Unix (see ufs.UnixFS.SetImpersonator).
func (fs *Filesystem) SetImpersonator(fn func(func() error) error) {
	fs.unixFS.UnixFS.SetImpersonator(fn)
}
