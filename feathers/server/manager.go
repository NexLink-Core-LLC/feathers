...
func (m *Manager) InitServer(data remote.ServerConfigurationResponse) (*Server, error) {
	...
	// The execution backend is selected at compile time via build tags: the
	// Docker environment on Linux, the native Job Object environment on Windows.
	// See server/environment_notwindows.go and server/environment_windows.go.
	...
	if env, err := createEnvironment(s.ID(), s.Config().Container.Image, envCfg); err != nil {
		return nil, err
	} else {
		s.Environment = env
		s.StartEventListeners()
		// Wire per-operation filesystem impersonation (on Windows this makes the
		// file manager / SFTP perform disk access as the server's restricted
		// account; no-op elsewhere).
		applyFilesystemIsolation(s.fs, s.Environment)
	}
